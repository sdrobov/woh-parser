...
class SiteParser {
  constructor (siteId, settings, mysqlConnection, lastPostDate) {
    ...
    this.tagsWhitelist =
      this.settings.tagsWhitelist || JSON.parse(env.TAGS_WHITELIST)
    this.contentRegexps =
      this.settings.contentRegexps || JSON.parse(env.GLOBAL_CONTENT_REGEXP)
  }
  ...
  parseDom (url) {
    ...
    return JSDOM.fromURL(mainUrl, {
      ...
      let titles =
        dom.window.document.querySelectorAll(this.settings.titlesSelector) || []
      let dates = [].map.call(
      ...
      if (!skipped && this.settings.nextSelector) {
        ...
        let nextUrl = dom.window.document.querySelector(
  ...
  getPage (url, contentAdd, firstImage) {
    return JSDOM.fromURL(url, {
      ...
      let content = dom.window.document.querySelector(
      ...
      if (content) {
        ...
        if (this.settings.nextContentSelector) {
          let nextPage = dom.window.document.querySelector(
            this.settings.nextContentSelector
          )
          if (nextPage) {
